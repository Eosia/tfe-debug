...
<div class="my-2 flex justify-content-start">
...
<div class="mt-5">
    @foreach($jobs as $job)
        ...
        @auth
            @if($job->status != 0 && $job->moderate != 1)
            
                <!-- si étudiant retourne annonces de recruteurs -->
                @if(Auth::user()->role_id != 3)
                    @if($job->user->role_id != 2)
                        <livewire:job :job="$job"/>
                    @endif

                <!-- si recruteur retourne annonces d'étudiants -->
                @elseif(Auth::user()->role_id != 2)
                    @if($job->user->role_id != 3)
                        <livewire:job :job="$job"/>
                    @endif
                @else
                    <livewire:job :job="$job"/>
    ...
    <div class="h-screen w-screen bg-gray-100 flex items-center">
        <div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
            <div class="max-w-md">
                ...
                <p class="text-2xl md:text-3xl font-light leading-normal">Aucun résultat pour cette recherche</p>
                <p class="mb-8">Veuillez séléctionner un autre critère.</p>

                <a href="{{ route('home.index') }}" class="btn btn-md px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">
                    Retour aux stages
                </a>
            </div>
            <div class="max-w-lg">
